...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        ArrayList<String> paragarph = readWordsFromNewFile("res/paragraph.txt");
        HashMap<String , Integer> paraCounter = buildHashMapFromNewFile(paragarph); //create a hashmap to count the occurance of each word
        createHTMLFileFromNewHashMap(paraCounter);
        ArrayList <ParagraphFrequency> paragraphListFromHashmap  = new ArrayList <>();
        paragraphListFromHashmap = populateParagraphFrequencyArrayFromHashMap(paraCounter);
        Collections.sort(paragraphListFromHashmap); 
        createSortedParagraphHTMLFile(paragraphListFromHashmap);

    }

/** Step 19
 * This is the function that creates a HTML file using a ArrayList<ParagraphFrequency> that displays a sorted table of words and count variables from the instances
 * @param paragraphList
 * @throws IOException
 */
private static void createSortedParagraphHTMLFile (ArrayList <ParagraphFrequency> paragraphList) throws IOException
{
    File file = new File ("res/sortedParagraph.html");
    ...
    for (ParagraphFrequency word: paragraphList) //makes a for each loop to use the keys from .keySet() of the hashmap paraCounter 
    {
        builder.append ("<tr>");
    ...
    /**step 18
     *  function to populate an ArrayList<ParagraphFrequency> from the hashmap
 * @param paraCounter this is the passed HashMap
 * @return returns the populated array paragraphList
 */
private static ArrayList <ParagraphFrequency>  populateParagraphFrequencyArrayFromHashMap(HashMap<String, Integer> paraCounter) 
{
    ArrayList <ParagraphFrequency> paragraphList  = new ArrayList <>();
    for (String key : paraCounter.keySet())
    {
        ParagraphFrequency count = new ParagraphFrequency(key,paraCounter.get(key));//assigns the value of hashmap to a new instance of class
        paragraphList.add(count);//assigns the values of the new instance of class to the array
    }
...
//step 16
     ...
     * @param paraCounter 
     */
    private static void createHTMLFileFromNewHashMap(HashMap<String, Integer> paraCounter)
    {
        ...
        try {
            ...
            builder.append("<table>"); 
            for (String key: paraCounter.keySet()) //makes a for each loop to use the keys from .keySet() of the hashmap wordCounter 
            {
                builder.append ("<tr>");
                builder.append ("<td>"+ key + "</td>");
                builder.append ("<td>"+ paraCounter.get(key) + "</td>"); // gets the value associated with each key 
                builder.append ("</tr>");
            }
...
private static ArrayList <WordFrequency>  populateArrayFromHashMap(HashMap<String, Integer> wordCounter) 
     ...
     * Step 14
     * @param fileName
     * @return the array created from reading the file after removing punctuation and spaces
     */
    private static ArrayList<String> readWordsFromNewFile(String fileName)//how to read a txt file and save contents to an Array
    {
        File file = new File (fileName);//declare a file
        ArrayList<String> wordList = new ArrayList<>(); //declare an array
 ...
 * This is the function that will build a HashMap out of the arrayList<String> with the word as key and count of word as value
     ...
     * @return the hashMap
     */
    private static HashMap<String, Integer> buildHashMapFromNewFile(ArrayList<String> words) 
        {
            HashMap<String,Integer> wordCounter = new HashMap<>(); //declaration
